...
impl ProductHelper {
    fn new() -> ProductHelper {
        ProductHelper {
            i: 0,
            values: [0, 0, 0, 0],
        }
    }
    fn add_val_and_get_product(&mut self, x: u64) -> u64 {
        self.values[self.i] = x;
        self.i = (self.i + 1) & 0x3;
        self.values.iter().fold(1, |acc, x| acc * x)
    }
}
fn product_in_grid() -> u64 {
    let m = vec![
        vec![
    ...
    let max_col = m.len();
    let max_row = m[0].len();
    let mut max_product = m[0][0];
    {
        let mut update_max = |x| {
            if x > max_product {
                max_product = x;
            }
        };
        ...
        // horizontal
        for r in 0..max_row {
            let mut h = ProductHelper::new();
            for c in 0..max_col {
                update_max(h.add_val_and_get_product(m[r][c]));
            }
        }

        /* do some "invisible zero padding", e.g. turn
        a,b,c,d,e,f,             0,0,0,0,a,b,c,d,e,f,
        g,h,i,j,k,l,     into    0,0,0,g,h,i,j,k,l,0,
        m,n,o,p,q,r,             0,0,m,n,o,p,q,r,0,0,
        s,t,u,v,w,x,             0,s,t,u,v,w,x,0,0,0,
        and then do the vertical operations.
        */
        // combined vert, down-left, and down-right.
        for _c in 0..(2 * max_col) {
            let mut vert = ProductHelper::new();
            let mut dnrt = ProductHelper::new();
            let mut dnlt = ProductHelper::new();
            for r in 0..max_row {
                // vertical:
                if _c < max_col {
                    update_max(vert.add_val_and_get_product(m[r][_c]));
                }

                // down_right:
                if _c + r >= max_col && _c + r - max_col < max_col {
                    update_max(
                        dnrt.add_val_and_get_product(m[r][_c + r - max_col]),
                    );
                }

                // down_left:
                if _c >= r && _c - r < max_col {
                    update_max(dnlt.add_val_and_get_product(m[r][_c - r]));
                }
            }
        }
    }
...
#[test]
fn euler_011() {
    // max horiz: 48477312 @ (08,13) [78, 78, 96, 83]
    // max vert:  51267216 @ (09,15) [66, 91, 88, 97]
    // max dn-rt: 40304286 @ (19,12) [94, 99, 71, 61]
    // max dn-lt: 70600674 @ (15,03) [87, 97, 94, 89]
